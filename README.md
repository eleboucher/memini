...
| `MEMINI_RERANK_POOL`             | `0`                      | how many composite-ranked candidates reach the reranker before the result is truncated to the recall limit. `0` reranks exactly the limit, which reorders the results but can never surface a memory that ranked below them — that rescue is most of a cross-encoder's value, so a deployment with a reranker wants this set (~`50`, the retrieval pool size, is the natural ceiling). Cost is linear: one forward pass per candidate                                                                                           |
| `MEMINI_RERANK_MAX_BATCH_CHARS`  | `6000`                   | cap the total query+documents characters per `/rerank` request; the pool is split across multiple **serial** requests when it would exceed this. This is an HTTP payload guard, not a context-window guard — a `/rerank` server scores each (query, document) pair in its own forward pass, so the model context bounds a pair, not the batch. Raise it (e.g. `120000`) when using `MEMINI_RERANK_POOL`, or a deep pool shards into many round trips and blows `MEMINI_RERANK_TIMEOUT`. `0` disables                            |
...
| `MEMINI_API_KEY`                 | —                        | the **admin/bootstrap key**: if set, required as a bearer token (also gates `/metrics`), and the only credential that can manage other keys via `/v1/keys`. See [Named API keys](#named-api-keys)                                                                                                                                                                                                                                                                                                                               |
| `MEMINI_API_KEYS_FILE`           | —                        | path to a declarative YAML file of named API keys, loaded once at boot (fail-loud validation). See [docs/api-keys.md](docs/api-keys.md)                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `MEMINI_HOME`                    | —                        | **client-side.** The caller's personal namespace (recommended `personal/<name>`), sent as `X-Memini-Home` on every call; merges durable (semantic/procedural) memories read-only into every recall/briefing's read set, and is where `visibility:"personal"` writes land. Resolved by the plugin like `MEMINI_NAMESPACE`; on stdio MCP (`memini mcp`) the server reads this env var directly, since stdio has no headers. Empty disables it (no home leg; `visibility:"personal"` errors). See [docs/scopes.md](docs/scopes.md) |
| `MEMINI_CASCADE`                 | `true`                   | server-wide switch for the ancestor/home/link read cascade. `false` restores pre-cascade isolation: a read sees only the request namespace (and its subtree when `scope: "everywhere"`), and `scope: "full"`/`"everywhere"` add no cascade legs. The escape hatch for upgraders who want the old isolation default without setting `scope: "project"` on every call. See [docs/scopes.md](docs/scopes.md#knobs)                                                                                                                 |
| `MEMINI_LOG_LEVEL`               | `info`                   | `debug` / `info` / `warn` / `error`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `MEMINI_LOG_FORMAT`              | `json`                   | `json` or `text`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |

### Named API keys

Beyond the single shared `MEMINI_API_KEY`, memini supports multiple named API
keys — each with its own home namespace and default namespace, managed
imperatively (`memini key add|rm|ls`, or `/v1/keys` when an admin key is
configured) or declaratively via `MEMINI_API_KEYS_FILE` for GitOps/SOPS
deployments. See [docs/api-keys.md](docs/api-keys.md) for the full model,
including how a key's home/default bindings interact with per-request
headers and the bootstrap/lockout behavior.
...
(or another one, on a durable write, when `visibility` says so — see
[docs/scopes.md](docs/scopes.md)). A read (`memory_recall`, `memory_briefing`,
`memory_answer`) can see more than that: its **read set** is the namespaces a given
call actually searches, composed from the request namespace plus its ancestor
cascade, unless a per-call override replaces it. This keeps writes simple and
isolated while letting reads pull in related context automatically.

**Default read set** (`scope: "full"`, the default): the request namespace, plus

- every ancestor namespace (nearest first), durable tiers only — `acme/phoenix/api`
  also reads `acme/phoenix` and `acme`, with no configuration required,
- the caller's **home** namespace (durable tiers only), when `MEMINI_HOME` /
  `X-Memini-Home` is set, and
- any stored [namespace links](docs/scopes.md#links) from the request namespace
  (durable tiers only by default).

`scope: "project"` skips the cascade entirely (primary only); `scope: "everywhere"`
adds the request namespace's own subtree on top of the full cascade (`work/memini`
also reads `work/memini/orchestrator`, `work/memini/reviewer`, ... — at the full
tier filter, since a subtree read is the caller's own namespace, not a cascade leg).
Setting `MEMINI_CASCADE=false` turns the cascade off server-wide, making every read
default to project-only isolation — the escape hatch for upgraders who relied on the
...
caller's home namespace protected first) with a logged warning.

| Mechanism                                   | Contributes                      | Tier access                                               |
| ------------------------------------------- | -------------------------------- | --------------------------------------------------------- |
| Request namespace                           | the namespace itself             | the request's own tier filter                             |
| Ancestor cascade (`scope: "full"`, default) | every path prefix, nearest first | durable tiers only                                        |
| Home (`MEMINI_HOME`)                        | the caller's personal namespace  | durable tiers only                                        |
| Namespace links                             | linked namespaces                | durable tiers only (link can narrow further, never widen) |
| `scope: "everywhere"`                       | the request namespace's subtree  | the request's own tier filter                             |
| `namespaces: [...]` (per-call)              | exactly the listed namespaces    | the request's own tier filter                             |

See [docs/scopes.md](docs/scopes.md) for the full write/read data flow, worked
examples, the isolation contract, and namespace links.
...
```
work                     # work-domain facts, read by every work/... namespace automatically (ancestor cascade)
work/<project>/<agent>   # per-project + per-agent isolation
personal/<name>          # the operator's home namespace (MEMINI_HOME), follows them everywhere
```

A recall from `work/memini` automatically reads `work`'s durable facts via the
ancestor cascade — no configuration needed, and no `_shared` sibling convention to
maintain. With `scope: "everywhere"` it also reads its own nested agent namespaces
(`work/memini/reviewer`, ...); `personal/*` stays untouched unless linked. Set
`MEMINI_HOME=personal/<name>` once per operator to get a personal namespace that
follows them across every project, replacing the old instance-global namespace with
a per-person equivalent. See [docs/scopes.md](docs/scopes.md) for the full model.
