...
| `MEMINI_RERANK_TIMEOUT`          | `3s`                     | per-recall bound on the reranker call; on timeout recall falls back to the composite order so it never blocks on a slow/restarting reranker                                                                                                                                                                                                                 |
...
| `MEMINI_CONSOLIDATE_QUEUE_CAP`   | `1024`                   | bound on the async consolidation queue; writes never block on it (jobs are dropped, best-effort, when it is full)                                                                                                                                                                                                                                           |
...
| `MEMINI_TOMBSTONE_TTL`           | `0`                      | sweeper hard-deletes superseded (tombstoned) memories last updated before `now - TTL`, reclaiming space. `0` keeps tombstones indefinitely (still excluded from recall). The one irreversible maintenance action — enable with care.                                                                                                                        |
| `MEMINI_DEMOTE_AFTER`            | `0`                      | sweeper demotes never-recalled, low-importance durable memories older than this back to the episodic tier so unused debris ages out. `0` disables.                                                                                                                                                                                                          |
| `MEMINI_DEDUP_INTERVAL`          | `24h`                    | how often the periodic store-wide dedup pass collapses near-duplicate memories into one representative per cluster (the rest are tombstoned, reversibly). Runs daily by default; set `0` to disable. Also exposed on-demand via `POST /v1/dedup`.                                                                                                           |
