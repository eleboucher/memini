...
| `MEMINI_MERGE_HINT_MIN_SCORE`     | `0`                      | when a write's nearest same-tier memory scores at/above this, the write still stores but the response carries a non-destructive **merge hint** so the caller (e.g. the model via `memory_remember`) can merge into the near-duplicate. Off by default; **recommended opt-in value ~`0.625`** — the dedup bench (`bench/dedup_test.go`) measured ~85% near-dup recall at ~1% false hint there. Adds a per-write vector lookup; must stay above `MEMINI_WRITE_DEDUP_MIN_SCORE` |
