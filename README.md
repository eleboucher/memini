...
| `MEMINI_MERGE_HINT_MIN_SCORE`     | `0.625`                  | when a write's nearest same-tier memory scores at/above this, the write still stores but the response carries a non-destructive **merge hint** so the caller (e.g. the model via `memory_remember`) can merge into the near-duplicate. On by default (set `0` to disable); `0.625` is the dedup-bench sweet spot (~85% near-dup recall, ~1% false hint). **Durable tiers only** (semantic/procedural) — episodic/working writes skip the lookup. Adds a per-write vector lookup on durable writes and **must stay above `MEMINI_WRITE_DEDUP_MIN_SCORE`** — if you set write-dedup ≥ `0.625`, raise this or lower that |
| `MEMINI_AUTO_SUPERSEDE_MIN_SCORE` | `0`                      | upper dedup gate (**destructive**): a write whose nearest same-tier memory scores at/above this tombstones the old one. Off by default — the bench found no threshold with ~0% false-deletion at useful recall; must be `> MEMINI_MERGE_HINT_MIN_SCORE` when set                                                                                                                                                                                                                                                                                                                                                      |
...
| `MEMINI_RECALL_SEMANTIC_RESERVE`  | `2`                      | guarantee up to this many recall slots for durable tiers (semantic/procedural) so episodic turn-capture volume can't crowd consolidated facts out of recall; runs after the relevance gate, evicting only the weakest in-window episodic (`0` disables)                                                                                                                                                                                                                                                                                                                                                               |
