...
func (s *Service) Recall(ctx context.Context, in RecallInput) ([]store.Scored, error) {
	...
	// The token budget trims last, AFTER the floor and expansion, so it sees the
	// list the caller would otherwise receive. It trims BEFORE reinforcement and
	// logging: a result the budget dropped never reached the caller, so it must
	// neither count as used nor appear as served — the event instead records the
