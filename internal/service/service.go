...
func (s *Service) consolidate(ctx context.Context, m *memory.Memory) (*memory.Memory, bool, error) {
	...
	if err != nil {
		return nil, false, fmt.Errorf("remember: consolidate: %w", err)
	}
...
func (s *Service) reinforce(ctx context.Context, namespace string, results []store.Scored) {
	now := s.now()
	byExpiry := map[time.Duration][]string{}
	for _, r := range results {
		byExpiry[r.Memory.Tier.DefaultTTL()] = append(byExpiry[r.Memory.Tier.DefaultTTL()], r.Memory.ID)
	}
	for ttl, ids := range byExpiry {
