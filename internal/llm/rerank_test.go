...
func cands(ids ...string) []RerankCandidate {
	out := make([]RerankCandidate, len(ids))
	for i, id := range ids {
		out[i] = RerankCandidate{ID: id, Content: "content of " + id}
	}
	return out
}
...
func TestRerankReordersAndAppendsOmitted(t *testing.T) {
	...
	r := NewReranker(&fakeCompleter{reply: "3, 1"})
...
func TestRerankNoneKeepsOriginalOrder(t *testing.T) {
	r := NewReranker(&fakeCompleter{reply: "none"})
...
func TestRerankIgnoresGarbageAndOutOfRange(t *testing.T) {
	...
	r := NewReranker(&fakeCompleter{reply: "garbage 2 99 2 banana"})
	got, _ := r.Rerank(context.Background(), "q", cands("a", "b", "c"))
