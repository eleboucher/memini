...
func (s *Store) PutPins(ctx context.Context, entries []store.Pin) error {
	...
	for _, e := range entries {
		...
		err := tx.QueryRowContext(ctx,
			`SELECT created_at, created_by FROM pins WHERE key=?`, e.Key).Scan(&existingCreated, &existingCreatedBy)
		switch {
		case errors.Is(err, sql.ErrNoRows):
			// Insert path: use the incoming created/created_by as-is.
		case err != nil:
			return fmt.Errorf("sqlitevec: lookup pin %q: %w", e.Key, err)
		default:
			...
			if err != nil {
				return fmt.Errorf("sqlitevec: parse existing pin created_at for %q: %w", e.Key, err)
			}
		...
		if _, err := tx.ExecContext(ctx,
			...
			return fmt.Errorf("sqlitevec: put pin %q: %w", e.Key, err)
		}
	}
	return tx.Commit()
}
...
func (s *Store) GetPins(ctx context.Context, keys []string) ([]store.Pin, error) {
	...
	q := `SELECT key, namespace, note, created_by, created_at, updated_at FROM pins WHERE key IN (` +
		placeholders(len(keys)) + `)`
	rows, err := s.db.QueryContext(ctx, q, keysToArgs(keys)...)
	if err != nil {
		return nil, fmt.Errorf("sqlitevec: get pins: %w", err)
	}
...
func (s *Store) DeletePins(ctx context.Context, keys []string) (int64, error) {
	...
	q := `DELETE FROM pins WHERE key IN (` + placeholders(len(keys)) + `)`
	res, err := s.db.ExecContext(ctx, q, keysToArgs(keys)...)
	if err != nil {
		return 0, fmt.Errorf("sqlitevec: delete pins: %w", err)
	}
...
func (s *Store) ListPins(ctx context.Context) ([]store.Pin, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlitevec: list pins: %w", err)
	}
	return scanPins(rows)
}
...
func (s *Store) RenamePinNamespaces(ctx context.Context, from, to string) error {
	...
	_, err := s.db.ExecContext(ctx, `UPDATE pins SET namespace=? WHERE namespace=?`, to, from)
	if err != nil {
		return fmt.Errorf("sqlitevec: rename pin namespaces: %w", err)
	}
	return nil
}
...
// scanPins reads every remaining row of rows into
// Pins, closing rows before returning.
func scanPins(rows *sql.Rows) ([]store.Pin, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, fmt.Errorf("sqlitevec: parse pin created_at: %w", err)
		}
		ut, err := time.Parse(time.RFC3339Nano, updated)
		if err != nil {
			return nil, fmt.Errorf("sqlitevec: parse pin updated_at: %w", err)
		}
		e.CreatedAt = ct.UTC()
		e.UpdatedAt = ut.UTC()
		out = append(out, e)
