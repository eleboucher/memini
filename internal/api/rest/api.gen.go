...
const (
	SettingsDefaultsResponseManagedByApi SettingsDefaultsResponseManagedBy = "api"
	SettingsDefaultsResponseManagedByEnv SettingsDefaultsResponseManagedBy = "env"
)
...
func (e SettingsDefaultsResponseManagedBy) Valid() bool {
	switch e {
	case SettingsDefaultsResponseManagedByApi:
		return true
	case SettingsDefaultsResponseManagedByEnv:
		return true
	default:
		return false
	}
...
type Memory struct {
	...
	// MergeHint Optional. Present only on POST /v1/memories responses when the write's nearest same-tier candidate scored at/above MEMINI_WRITE_DEDUP_SCORE and MEMINI_WRITE_DEDUP_ACTION is "hint".
...
// SettingsDefaultsResponse The server's global default ClientSettings (fully resolved — every field present) plus who manages this layer, so the admin UI can render an env-managed layer read-only.
