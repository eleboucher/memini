...
| `MEMINI_INJECT_RECALL_MIN_SCORE` | `0`      | Floor on the final ranked (composite) score — the score shown in the activity feed. Enforced server-side via `min_rank_score` (floored hits appear in the feed marked as filtered); against older servers the plugin filters client-side instead. |
...
| `MEMINI_INJECT_PRETOOL_MIN_SCORE` | `0`                                        | Floor on the final ranked (composite) score — the score shown in the activity feed. Enforced server-side via `min_rank_score` (floored hits appear in the feed marked as filtered); against older servers the plugin filters client-side instead. |
