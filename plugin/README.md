...
| `MEMINI_TIMEOUT_MS`    | `30000`                 | hooks       | how long one memini call may take before the hook gives up. Also a server-pushable setting (`request_timeout_ms`). Keep it **above** the server's `MEMINI_RERANK_TIMEOUT`: the server degrades a slow rerank to composite order, but a client that hangs up first gets nothing at all. The handshake keeps its own ~2.5s timeout. |
...
| `MEMINI_CAPTURE_USER_MAX_CHARS`      | `1000`  | truncate the user side of a captured turn to this many characters, marking the cut; `0` captures it whole. Server-resolved by default — the server knows its own store and recall budget.          |
...
| Env var                          | Default | Description                                                                                                                                                                                                                                                                  |
| -------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MEMINI_RECALL`                  | on      | Master switch for recall-driven injection. `MEMINI_RECALL=0` disables this hook AND the PreToolUse recall below.                                                                                                                                                             |
...
| `MEMINI_INJECT_PRETOOL_MIN_SCORE` | `0.5`                          | Floor on the composite post-rerank score — the final score shown in the activity feed. Enforced server-side via `min_rank_score` (floored hits appear in the feed marked as filtered); against older servers the plugin filters client-side instead. `0` disables the floor. |
| `MEMINI_INJECT_PRETOOL_TOOLS`     | `Read\|Write\|Edit\|MultiEdit` | Pipe- or comma-separated tool allowlist override. `Glob`/`Grep` are out of the default — pattern-derived queries ("Grep on \<pattern\>") are near-zero-signal and each ungated call costs a server embed+rerank — list them to restore the old behavior.                     |
