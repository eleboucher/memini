...
| ------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MEMINI_CASCADE`          | live, **server-side** | Default `true`. The server-wide switch for the ancestor/home/link cascade. Set `false` to restore pre-cascade isolation: the default read set becomes the request namespace (and its subtree, when asked) only, and `scope:"full"`/`"everywhere"` no longer add the cascade legs. An escape hatch for operators upgrading who want the old isolation default without setting `scope:"project"` on every call. Per-call `scope` and explicit `namespaces` still work either way. |
