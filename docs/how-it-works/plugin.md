...
Claude Code wires all seven events; Codex wires six (it has no reliable final-session event, so the session-end digest is Claude-only and Codex relies on the rolling Stop checkpoints instead). Cursor wires all seven under its native camelCase names (`sessionStart`, `beforeSubmitPrompt`, `preToolUse`, `postToolUse`, `stop`, `preCompact`, `sessionEnd`), with the caveats listed under Known limitations.
...
| SessionEnd (Claude, Cursor) | on cache miss                         | Writes the final session digest, superseding the Stop checkpoint; deletes all per-session state                                      | Nothing                                                 |
...
That answer is cached per session for ten minutes, keyed by the harness process id, the working directory, and a hash of the facts — so a changed remote or a moved directory is a cache miss, not a stale hit. Cursor spawns each hook under a fresh parent process, so there the cache keys on the payload's `conversation_id` instead (`hookCacheKey` in `_shared.mjs`). The hot-path hooks (UserPromptSubmit, PreToolUse, PostToolUse) read only this cache and make zero resolution round-trips. Stop refreshes the cache on a miss once per turn, which is what keeps it warm through a long session.
...
- **Cursor's MCP namespace is static, localhost-only by default.** Plugin-bundled MCP config does not interpolate `${env:...}` (the literal placeholder is sent as the bearer and rejected) — only dashboard-set plugin variables substitute there. The bundled entry is a bare `http://localhost:8080/mcp`; auth or a remote URL goes through a user-scope `~/.cursor/mcp.json` override, where `${env:...}` works. No `headersHelper` equivalent exists, so per-project resolution applies to the hooks' REST traffic only.
- **Cursor's `beforeSubmitPrompt` has no documented context-injection output.** The hook emits the sessionStart-style `additional_context` field best-effort; a Cursor build that ignores it silently loses per-prompt recall.
- **Cursor's `sessionEnd` fires on window close**, after shell teardown, so the final session digest can be skipped — the rolling Stop checkpoints bound the loss. Transcript-dependent features (turn capture, inline `<memory>` extraction, the auto-save nudge) stay Claude-only: Codex's transcript is unstable by contract, Cursor's is not Claude Code's JSONL, and Cursor's stop `followup_message` would auto-submit the nudge as a user message.
...
- `plugin/hooks/hooks.claude.json` — the seven Claude Code events; `plugin/hooks/hooks.codex.json` — the six Codex events; `plugin/hooks/hooks.cursor.json` — the seven Cursor events in Cursor's native camelCase format, commands addressed via `${CURSOR_PLUGIN_ROOT}`. Deliberately not named `hooks.json`: Claude Code loads that default path in addition to the manifest-declared one, so a file there would run every hook twice. The Codex file's top level accepts only `description` and `hooks` — Codex parses it into a `deny_unknown_fields` struct, so a `"//"` comment key rejects the entire file at startup; Claude Code keeps loading its file but warns `hooks.json: unknown key "//" ignored`, so neither file carries one. Each Codex handler also carries a `commandWindows` invoking `node` directly: Codex runs hook commands through `cmd.exe /C` on Windows, which cannot execute the POSIX `scripts/run.sh`. Claude Code has no equivalent field, so `hooks.claude.json` stays on `run.sh`.
- `plugin/.mcp.json` — the Claude Code MCP registration: `${MEMINI_BASE_URL:-http://localhost:8080}/mcp` (flat expansion; the `/mcp` suffix lives outside the braces) plus the `headersHelper` shell snippet. `plugin/.mcp.codex.json` — the static Codex form (`env_http_headers`). `plugin/.mcp.cursor.json` — the static Cursor form: a bare fixed localhost URL (plugin-bundled MCP config does not interpolate `${env:...}`; auth/remote via a user-scope `~/.cursor/mcp.json` override).
