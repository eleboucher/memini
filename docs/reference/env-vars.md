...
| `MEMINI_API_KEY`          | The bearer token this client sends. Absent means an unauthenticated request. The Claude Code plugin's SessionStart hook also mirrors it into `${XDG_CONFIG_HOME:-~/.config}/memini/credentials` (mode 0600, keyed by base URL) so the MCP `headersHelper` — which Claude Code ≥ 2.1.238 runs without inherited credential env vars — can still authenticate. An explicit `MEMINI_API_KEY=""` retires that stored copy (and says so on stderr); merely unsetting the variable leaves it alone, so a session launched without the key cannot strand the ones that depend on it — see [tri-state retirement](../../plugin/README.md#retiring-the-stored-credential-is-a-three-state-decision). |
...
| `MEMINI_TIMEOUT_MS`       | How long one memini call may take before the client gives up. Defaults to `30000`. The one variable that is **both** a bootstrap var and a [behavior setting](#4-behavior-settings-layered-server-data) (`request_timeout_ms`) — a request timeout is transport, and it has to exist _before_ the handshake that fetches settings. Keep it above the server's `MEMINI_RERANK_TIMEOUT`, or a slow reranker returns nothing instead of degrading to composite order.                                                                                                                                                                                                                          |
...
| `MEMINI_NAMESPACE_PREFIX` | A client-side override of the `namespace_prefix` behavior setting, sent as a fact. Prepended to the **derived** name (`<prefix>/<repo>`), so one credential can serve several namespace trees selected per shell/directory (e.g. a per-tree `.envrc`) — no pin, no second key. See [Behavior](#4-behavior-settings-layered-server-data).                                                                                                                                                                                                                                                                                                                                                    |
...
`request_timeout_ms` (`MEMINI_TIMEOUT_MS`) is the one setting that is **also** a
[bootstrap](#1-bootstrap-client-side-env-read-once) variable, and deliberately
so: a request timeout is transport, and it has to bound the very first call —
including the handshake's siblings — before any settings exist to read. Pushing
it from the server is what lets a deployment that runs a slow cross-encoder
reranker raise every client's ceiling at once, instead of asking each user to
export a variable. Keep it above the server's
[`MEMINI_RERANK_TIMEOUT`](configuration.md#memini_rerank_timeout): the server
bounds a slow rerank and degrades to composite order, but a client that hangs up
first receives nothing at all rather than an unranked result.
...
several namespace trees selected by directory: point a per-tree `.envrc` (or a shell
hook) at `export MEMINI_NAMESPACE_PREFIX=<prefix>` and every repo under that
tree resolves to `<prefix>/<repo>`, with the repo name derived from the git
