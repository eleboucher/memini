...
- [x] A session receives one bounded layered briefing automatically without requiring the model to spend a tool call.
- [x] Compaction clears context-coupled suppression state and re-injects a fresh briefing before the next model request.
- [x] Resume/reload behavior neither loses valid suppression state nor duplicates a briefing already present in intact context.
- [x] Turn capture occurs once after automatic retries, compaction retries, and queued continuations have settled.
- [x] Pre-compaction or shutdown checkpoint behavior preserves useful activity that ordinary turn text would otherwise lose, where Pi exposes enough evidence.
- [x] Lifecycle tests cover startup, resume, reload, compaction, retry, and shutdown behavior.
