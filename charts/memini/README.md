...
| prometheusRule | object | `{"enabled":false,"labels":{},"rules":{"embedErrors":{"enabled":true,"for":"10m","severity":"warning","window":"30m"},"recallDegraded":{"enabled":true,"severity":"warning","window":"30m"}}}` | Bundled Prometheus alerting rules, rendered as a PrometheusRule for the prometheus-operator (a custom chart key, not part of the common library). Requires the `monitoring.coreos.com` CRDs. The rules watch memini's own degradation counters: both conditions keep serving requests, so they surface as quietly worse recall rather than as errors. |
| prometheusRule.enabled | bool | `false` | Render the bundled alerting rules as a PrometheusRule. |
| prometheusRule.labels | object | `{}` | Extra labels for the PrometheusRule. Set the selector your Prometheus CR's `ruleSelector` matches on, e.g. `release: kube-prometheus-stack`. |
| prometheusRule.rules.embedErrors | object | `{"enabled":true,"for":"10m","severity":"warning","window":"30m"}` | An embed backend is returning errors. |
| prometheusRule.rules.embedErrors.for | string | `"10m"` | Hold time before firing; embed errors are often a brief blip. |
| prometheusRule.rules.embedErrors.window | string | `"30m"` | increase() lookback. |
| prometheusRule.rules.recallDegraded | object | `{"enabled":true,"severity":"warning","window":"30m"}` | Recall fell back to unranked order because the reranker or embedder was unavailable. |
| prometheusRule.rules.recallDegraded.window | string | `"30m"` | increase() lookback. |
