...
| controllers.main.containers.main.env.MEMINI_UI_ADDR | string | `":8081"` | Dedicated admin-UI listener. The UI shell embeds MEMINI_API_KEY so the browser can call /v1; serving it on its own port keeps that key off the main port (which then carries only the token-free API). Expose this port only on a trusted (LAN) gateway — see route.ui. "" serves the UI on the main port instead (the key then rides / on the main port). |
...
| route.main | object | `{"enabled":false,"hostnames":[],"kind":"HTTPRoute","parentRefs":[],"rules":[{"backendRefs":[{"identifier":"main","port":"http"}]}]}` | Public API + MCP surface, on the token-free `http` port. Enable, set the gateway in parentRefs and the hostname. Because MEMINI_UI_ADDR moves the token-embedding shell to its own port, this route can safely forward all of `http` (only /v1, /mcp, /healthz, /readyz live there — no API key). Add rules[].matches to scope it further if you want to keep the probes private. |
| route.ui | object | `{"enabled":false,"hostnames":[],"kind":"HTTPRoute","parentRefs":[],"rules":[{"backendRefs":[{"identifier":"main","port":"ui"}]}]}` | Admin UI, on the `ui` port (MEMINI_UI_ADDR) whose shell embeds the API key. Disabled by default; enable on installs that want the SPA through a Gateway, and wire it to an INTERNAL gateway so the key never leaves the LAN. If MEMINI_UI_ADDR is "" the UI rides the main port instead — then point this at `port: http` and treat that whole route as trusted. |
...
| service.main.ports.ui | object | `{"port":8081}` | Dedicated admin-UI port (MEMINI_UI_ADDR). Carries the token-embedding shell; wire route.ui to it and keep that route on an internal gateway. |
